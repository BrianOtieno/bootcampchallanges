    ...
    var methods = {
        ...
        init: function(options) {               
            ...
            if (!form.data('jqv') || form.data('jqv') == null ) {		
				...
				//$(".formError").delegate("click", function() {
            ...
            for (var i = 0; i < rules.length; i++) {
                ...
                switch (rules[i]) {
                    ...
                    case "creditCard":
                        ...
                        break;
                    case "usNumber":
                        errorMsg = methods._usNumber(field, rules, i, options);
                        break;   
                    case "time":
                        errorMsg = methods._isTime(field, rules, i, options);
                        break;    
                    case "atleastOneNum":
                        errorMsg = methods._atleastOneNum(field, rules, i, options);
                        break;    
        ...
        */
        ...
        _usNumber: function(field, rules, i, options) {
           var phone2 = /^(\([0-9]{3}\)[0-9]{3}-[0-9]{4}$)/;
           var usNumber = field.val();
           if (usNumber.match(phone2)) {
                
           } else {
                return 'Invalid Format <br/> Format (xxx)xxx-xxxx';
           }
        },
        _isTime: function (field, rules, i, options){
            var TimeRegEx = new RegExp(/^((1[012]|0?[1-9]){1}:(0?[1-5]|[0-6][0-9]){1}\s+(AM|PM){1}$)/);
            if (TimeRegEx.test(field.val())) {
                
            }else{
               return 'Time format xx:xx AM|PM';
            }
        },
        _atleastOneNum: function(field, rules, i, options) {
            //var num = (/[0-9]+/);
          //  var num =/^([a-zA-Z0-9]+)$/;
            var num = /^(?:[0-9]+[a-z]|[a-z]+[0-9])[a-z0-9]*$/i;
            var atleastOneNum = field.val();
            if (atleastOneNum.match(num)) { 
            } else {
                return 'At least one number and one character are required for password</br>';
            }
        },
        /**
